...
function forEachDirEnt(dir, f) {
  return Bindings$PesyBootstrapper.Fs.readdir(dir).then((function (files) {
...
function scanDir(dir, f) {
  return forEachDirEnt(dir, (function (entry) {
                ...
                return Curry._1(f, entryPath).then((function (param) {
                              return Bindings$PesyBootstrapper.Fs.isDirectory(entryPath).then((function (isDir) {
                                            if (isDir) {
...
function copyBundledTemplate(param) {
  var templatesDir = Path.resolve(__dirname, "..", "templates", "pesy-reason-template-0.1.0-alpha.2");
...
function subst(file) {
  return Bindings$PesyBootstrapper.Fs.readFile(file).then((function (b) {
                    return Promise.resolve(Buffer.from(substituteTemplateValues(b.toString())));
                  })).then((function (s) {
                  return Bindings$PesyBootstrapper.Fs.writeFile(file, s);
                })).then((function (param) {
                return Promise.resolve((Fs.renameSync(file, substituteFileNames(file).replace("-template", "")), /* () */0));
              }));
}

function substitute(projectPath) {
  return Promise.all(WalkSync(projectPath).filter((function (file_or_dir) {
...
function spinnerEnabledPromise(spinnerMessage, promiseThunk) {
  ...
  return Curry._1(promiseThunk, /* () */0).then((function (x) {
...
function setup(template, projectPath) {
  return bootstrap(projectPath, template).then((function (param) {
                          return spinnerEnabledPromise("\x1b[2mSetting up\x1b[0m ", (function (param) {
                                        return substitute(projectPath);
                                      }));
                        })).then((function (_arrayOfCompletions) {
                        return spinnerEnabledPromise("\x1b[2mRunning\x1b[0m esy install", (function (param) {
                                      return Bindings$PesyBootstrapper.ChildProcess.exec("esy i", {
                                                  cwd: projectPath
                                                });
                                    }));
                      })).then((function (param) {
                      return spinnerEnabledPromise("\x1b[2mRunning\x1b[0m esy pesy\x1b[2m and \x1b[0mbuilding project dependencies", (function (param) {
                                    return Bindings$PesyBootstrapper.ChildProcess.exec("esy pesy", {
                                                cwd: projectPath
                                              });
                                  }));
                    })).then((function (param) {
                    return spinnerEnabledPromise("\x1b[2mRunning\x1b[0m esy build", (function (param) {
...
var doc = "Specify URL of the remote template. This can be of the form https://repo-url.git#<commit|branch|tag>. Eg: https://github.com/reason-native-web/morph-hello-world-pesy-template#6e5cbbb9f28";

var v = {
  contents: undefined
};

var anonFun = /* String */Caml_chrome_debugger.variant("String", 4, [(function (template) {
        v.contents = template;
        return /* () */0;
      })]);
...
var anonFun$2 = /* Unit */Caml_chrome_debugger.variant("Unit", 0, [(function (param) {
...
Arg.parse(specList, (function (param) {
        return /* () */0;
      }), usageMsg);
